...
async function checkAllMarketBalances() {
  ...
  for (const [marketKey, market] of Object.entries(CONTRACTS.MARKETS)) {
    ...
    try {
      // Check SY balance
      const syContract = new ethers.Contract(market.syToken, erc20ABI, provider);
      const syBalance = await syContract.balanceOf(USER_ADDRESS);
      ...
      const formattedBalances = {
        sy: ethers.formatEther(syBalance),
        pt: ethers.formatEther(ptBalance),
        yt: ethers.formatEther(ytBalance)
      };
      ...
      console.log(`   ❌ Error reading ${marketKey}:`, error.message);
