...
async function main() {
  console.log("🚀 TESTING ALL WORKING MARKETS INCLUDING LSTBTC!");
  ...
  try {
    ...
    console.log("\n🔧 STEP 2: Finding ALL Working Markets (Including lstBTC)...");
    console.log("-".repeat(50));
      ...
      try {
        ...
        try {
          ...
          if (poolData.reserve0 > 0 && poolData.reserve1 > 0 && poolData.isActive) {
            const tokenName = getTokenName(marketInfo.underlying);
            marketTypes[tokenName]++;
            ...
            console.log(`✅ Working market found at index ${i} - ${tokenName}`);
          }
        } catch (error) {
    ...
    console.log("\n🔧 STEP 3: Testing Complete User Flow on ALL Working Markets...");
    console.log("-".repeat(50));
    ...
    for (let marketIndex = 0; marketIndex < workingMarkets.length; marketIndex++) {
      ...
      const tokenName = market.tokenName;
    ...
    console.log("\n💡 NOTE: lstBTC markets are now properly included and tested!");
